...
# Evita que Kivy escriba en ~/.kivy/logs (que está corrupto ahora mismo)
import os
os.environ["KIVY_NO_FILELOG"] = "1"
# Si en el futuro quieres logs a archivo en una carpeta propia, cambia a:
# os.environ.pop("KIVY_NO_FILELOG", None)
# os.environ["KIVY_LOG_DIR"] = "/home/pi/Desktop/Finale_Alva/logs/kivy"
# os.makedirs("/home/pi/Desktop/Finale_Alva/logs/kivy", exist_ok=True)
...
# Ruta del launcher (tu script bash local en la Pi)
LAUNCHER_PATH = "/home/pi/Desktop/Finale_Alva/launcher.sh"

# Indicador chico de Jetson/service
JETSON_LAMP_SIZE = (18, 18)      # tamaño del puntito
JETSON_LAMP_POS  = {"center_x": 0.5, "top": 0.97}  # centro arriba
LAMP_REFRESH_SEC = 3.0           # cada cuántos s se chequea estado

SERVICE_NAME = "finale.service"  # servicio en la Jetson

# ========= helpers =========
def run_launcher(cmd: str):
    """
    Play/Stop/Reset: control Jetson (servicio) + ejecutar launcher.sh local (Pi).
    Resto de comandos: se van a launcher.sh como siempre.
    """
    try:
        if cmd == "start":
            # 1) Jetson: arrancar servicio
            subprocess.Popen(
                ["/bin/bash", "-lc", "/usr/local/bin/finale-start"],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )
            # 2) Pi: ejecutar lógica local
            subprocess.Popen(
                ["/bin/bash", "-lc", f'"{LAUNCHER_PATH}" start'],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )

        elif cmd == "stop":
            # 1) Pi: parar lógica local primero (cierre limpio)
            subprocess.Popen(
                ["/bin/bash", "-lc", f'"{LAUNCHER_PATH}" stop'],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )
            # 2) Jetson: detener servicio
            subprocess.Popen(
                ["/bin/bash", "-lc", "/usr/local/bin/finale-stop"],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )

        elif cmd == "reset":
            # Parar ambos y volver a arrancar ambos (secuenciado en shell)
            subprocess.Popen(
                ["/bin/bash", "-lc",
                 f'("{LAUNCHER_PATH}" stop || true); /usr/local/bin/finale-stop || true; '
                 f'sleep 2; /usr/local/bin/finale-start; "{LAUNCHER_PATH}" start'
                ],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )

        else:
            # Comportamiento original (no tocamos tu lógica existente)
            subprocess.Popen(
                ["/bin/bash", "-lc", f'"{LAUNCHER_PATH}" {cmd}'],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )
...
def _jetson_connected() -> bool:
    """
    Conectividad básica a Jetson usando run_jetson; timeout local para no colgar la UI.
    """
    try:
        cmd = "timeout 3s /usr/local/bin/run_jetson 'echo ok'"
...
def _service_running() -> bool:
    """
    True si finale.service está activo en la Jetson; False si no.
    """
    try:
        cmd = f"timeout 3s /usr/local/bin/run_jetson 'sudo -n systemctl is-active --quiet {SERVICE_NAME}'"
        r = subprocess.run(["/bin/bash", "-lc", cmd],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return r.returncode == 0
    except Exception:
...
class BaseWorkPanel(FloatLayout):
    def __init__(self, status_pos=("center_x", 0.5, "top", 0.90), **kwargs):
        ...
        # Indicador chico (Jetson/servicio): centro arriba
        ...
        # Poll de estado Jetson/servicio
    ...
    # --- Indicador chico: polling estado real ---
    ...
    # callbacks base (solo UI + refresco de indicador)
    def on_start(self, *_):
        self._start_timer()
...
class Panel_Interfaz(BaseWorkPanel):
    def __init__(self, **kwargs):
        ...
        # Binds -> wrappers + launcher local (vía run_launcher)
