...
timest = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
CSV_PATH = os.getenv("CSV_INSTAGRAM_PATH") or f"/home/pi/Desktop/Finale_Alva/Nativo/Data/Instagram_Data_{timest}.csv"

# ===== Hora del TELÉFONO para CSV =====
def device_now_via_appium(driver) -> datetime:
    """Hora del dispositivo (vía 'mobile: shell date +%s'). Fallback: host."""
    try:
        res = driver.execute_script("mobile: shell", {
            "command": "date", "args": ["+%s"], "timeout": 5000, "includeStderr": True
        })
...
def timestamp_phone(driver) -> str:
    """Timestamp formateado (ms) usando hora DEL TELÉFONO."""
    return device_now_via_appium(driver).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
...
def seleccionar_media(driver, index):
    try:
        sel = esperar(driver, EC.presence_of_element_located((AppiumBy.ANDROID_UIAUTOMATOR,
...
def compartir_P(driver, tipo_metrica, red, lat, lon, inicio, tam_archivo, wait_for_id, wait_for_id2, wait_timeout=25):
    try:
        ...
        guardar_resultado(tipo_metrica, red, lat, lon, inicio, timestamp_phone(driver), "Successful", tam=tam_archivo)
    except TimeoutException:
        print("Timeout: no se detectó desaparición ni aparición del botón Enviar.")
        guardar_resultado(tipo_metrica, red, lat, lon, inicio, timestamp_phone(driver), "Failed", "Timeout", tam_archivo)

def compartir(driver, tipo_metrica, red, lat, lon, inicio, tam_archivo, wait_for_id, wait_timeout=5):
    try:
...
def enviar_contenido_F(driver, tipo, index, btn_id, wait_for_id):
    ...
    if not seleccionar_media(driver, index):
        # Nota: aquí no tenemos lat/lon ni inicio válidos -> se dejan vacíos como en tu original
        guardar_resultado(tipo, red, "", "", "", "", "Failed", "Item no found", "Desconocido")
        return
    lat, lon = get_location(udid)
    inicio = timestamp_phone(driver)
    tam_archivo = "2.3"
    if click_button(driver, btn_id, "Enviar foto", False, 6):
        compartir(driver, tipo, red, lat, lon, inicio, tam_archivo, wait_for_id)
    else:
        guardar_resultado(tipo, red, lat, lon, inicio, timestamp_phone(driver), "Failed", "No se encontró botón", tam_archivo)
...
def enviar_contenido_V(driver, tipo, index, btn_id, wait_for_id):
    ...
    inicio = timestamp_phone(driver)
    tam_archivo = "20"
    if click_button(driver, btn_id, "Enviar video", False, 6):
        time.sleep(2)
        compartir(driver, tipo, red, lat, lon, inicio, tam_archivo, wait_for_id, 25)
    else:
        guardar_resultado(tipo, red, lat, lon, inicio, timestamp_phone(driver), "Failed", "No se encontró botón", tam_archivo)
...
def enviar_mensaje_texto(driver, mensaje, red):
    ...
    if ingresar_texto(driver, "com.instagram.android:id/row_thread_composer_edittext", mensaje):
        ...
        if clic(driver, "com.instagram.android:id/row_thread_composer_send_button_container", "Enviar texto", timeout=7, mandatory=False):
            ...
            guardar_resultado(METRICAS["TEXT"], red, lat, lon, inicio, timestamp_phone(driver), "Failed", "No se encontró botón", tam=tam_archivo)
    else:
...
# === PARCHE MINIMO: verificación robusta del scrubber ===
def verificar_reproduccion_video(driver, duracion_segundos=15, check_interval=2.5):
    """
    Mantiene la lógica original basada en 'scrubber', pero evita falsos negativos:
    - Espera a que aparezca el scrubber (y hace un tap al centro si no está).
    - No falla por una sola lectura igual: exige varias lecturas iguales (~5s).
    - Tolera lecturas vacías/no numéricas del atributo 'text'.
    """
    # 1) Asegúrate de que haya scrubber; si no, intenta activar la reproducción con un tap
    try:
        WebDriverWait(driver, 8).until(
            EC.presence_of_element_located((AppiumBy.ID, "com.instagram.android:id/scrubber"))
        )
    except Exception:
        try:
            size = driver.get_window_size()
            # intento de "tap" al centro; si tu cliente no soporta .tap(), un swipe cero-distancia suele servir
            try:
                driver.tap([(size['width']//2, size['height']//2)])
            except Exception:
                driver.swipe(size['width']//2, size['height']//2, size['width']//2, size['height']//2, 100)
    ...
    tiempo_total = 0.0
    progreso_anterior = None
    intentos_sin_scrubber = 0
    iguales_consecutivos = 0
    # tolera ~5s de “igualdad” aunque el video esté avanzando en saltos gruesos
    max_iguales = max(1, int(5 / check_interval))

    while tiempo_total < duracion_segundos:
        try:
            scrubber = driver.find_element(AppiumBy.ID, "com.instagram.android:id/scrubber")
            txt = scrubber.get_attribute("text") or ""
            try:
                progreso_actual = float(txt)
            except Exception:
                progreso_actual = None

            if progreso_actual is None:
                intentos_sin_scrubber += 1
                if intentos_sin_scrubber >= 3:
                    return False
            else:
                intentos_sin_scrubber = 0
                if progreso_anterior is not None and progreso_actual == progreso_anterior:
                    iguales_consecutivos += 1
                    if iguales_consecutivos >= max_iguales:
                        return False
                else:
                    iguales_consecutivos = 0
...
def obtener_tamano_archivo_android(nombre_archivo):
    ruta = f"/sdcard/DriveTest/{nombre_archivo}"
...
def test_instagram():
    ...
    print("Cargando feed...")
    lat, lon = get_location(udid)
    inicio = timestamp_phone(driver)  # HORA TELÉFONO
    try:
        ...
        guardar_resultado(METRICAS["FEED"], red, lat, lon, inicio, timestamp_phone(driver))
    except:
        guardar_resultado(METRICAS["FEED"], red, lat, lon, inicio, timestamp_phone(driver), "Failed", "Timeout")
    ...
    # if click_tab_icon(driver, "com.instagram.android:id/tab_icon", 3, "Crear", 6) and asegurar_publicacion_activa(driver):
    #     clic(driver, "com.instagram.android:id/gallery_folder_menu_tv")
    #     el = esperar(driver, EC.presence_of_element_located((AppiumBy.ANDROID_UIAUTOMATOR,
    #         f'new UiSelector().resourceId("IgLazyColumn")'
    #         f'.childSelector(new UiSelector().description("Photos").instance(1))')))
    #     el.click()
    ...
    #                         "com.instagram.android:id/row_pending_media_reshare_button", 7)
    ...
    click_tab_icon(driver, "com.instagram.android:id/tab_icon", 4, "Reels", 5)
    if buscar_reel_con_scrubber(driver):
        lat, lon = get_location(udid)
        inicio = timestamp_phone(driver)  # HORA TELÉFONO
        if verificar_reproduccion_video(driver, duracion_segundos=10, check_interval=2.5):
            guardar_resultado(METRICAS["RVIDEO"], red, lat, lon, inicio, timestamp_phone(driver), "Successful", "", "0.0")
        else:
            guardar_resultado(METRICAS["RVIDEO"], red, lat, lon, inicio, timestamp_phone(driver), "Failed", "Reel no válido", "0.0")
    ...
    try:
        click_tab_icon(driver, "com.instagram.android:id/tab_icon", 0, "Home", 5)
        ...
        mensaje = "Hola Instagram"
        tam_archivo = f"{len(mensaje)} caracteres"
        if ingresar_texto(driver, "com.instagram.android:id/row_thread_composer_edittext", mensaje, "Campo texto"):
            lat, lon = get_location(udid)
            inicio = timestamp_phone(driver)  # HORA TELÉFONO
            if clic(driver, "com.instagram.android:id/row_thread_composer_send_button_container", "Botón enviar", 6, False):
                compartir(driver, METRICAS["TEXT"], red, lat, lon, inicio, tam_archivo, "com.instagram.android:id/action_icon")
            else:
                guardar_resultado(METRICAS["TEXT"], red, lat, lon, inicio, timestamp_phone(driver), "Failed", "Botón enviar no disponible", tam_archivo)
        else:
            guardar_resultado(METRICAS["TEXT"], red, "", "", "", "", "Failed", "Campo texto no encontrado", tam_archivo)
    except Exception as e:
        guardar_resultado(METRICAS["TEXT"], red, "", "", "", "", "Failed", "Unexpected error", tam_archivo)
...
def ejecutar_pruebas(n=1):
    for i in range(n):
        if detener:
            print(f"Detenido externamente en la iteración {i+1}")
            break
        print(f"\n Iteración {i+1}")
        try:
            continuar = test_instagram()
            if continuar is False:
                print(f"[INFO] Finalizando ejecución desde test_instagram en iteración {i+1}")
                cerrar_apps(["com.instagram.android"], udid)
                break
        except Exception as e:
