...
#CSV_PATH = f"C:/Users/Viva/3D Objects/Instagram_Data_{timestamp}.csv" 
#CSV_PATH = f"/media/pi/P_VAST/Instagram_Data_{timestamp}.csv"
CSV_PATH = f"/home/pi/Desktop/Testdrive/data/Instagram_Data_{timestamp}.csv"
...
def verificar_reproduccion_video(driver, duracion_segundos=10, check_interval=5):
    tiempo_total = 0
    progreso_anterior = -1
...
def test_instagram():
    ...
    try:
        ...
        ), 6) #TIEMPO DE ESPERA DE TIMEOUT DE CARGA DE FEED
        ...
        if exito:
            ...
            if clic(driver, "com.instagram.android:id/share_footer_button", "Compartir"):
                compartir_P(driver, METRICAS["POST"], red, lat, lon, inicio, tam_archivo,
                            ...
                            "com.instagram.android:id/row_pending_media_reshare_button", 6)#TIEMPO DE ESPERA DE TIMEOUT PARA PUBLICAR IMAGEN
    ...
    if buscar_reel_con_scrubber(driver):
        lat, lon = get_location(udid)
        inicio = timestamp()
        if verificar_reproduccion_video(driver): #TIEMPO DE REPRODUCIR VIDEO
    ...
    try:
        ...
        if ingresar_texto(driver, "com.instagram.android:id/row_thread_composer_edittext", mensaje, "Campo texto"):
            lat, lon = get_location(udid)
            inicio = timestamp()
            if clic(driver, "com.instagram.android:id/row_thread_composer_send_button_container", "Botón enviar", 6, False):#TIEMPO DE ESPERA DE TIMEOUT DE TEXTO
                compartir(driver, METRICAS["TEXT"], red, lat, lon, inicio, tam_archivo, "com.instagram.android:id/action_icon")
            else:
    ...
    try:
        enviar_contenido_F(driver, METRICAS["PHOTO"], 1, "com.instagram.android:id/direct_media_send_button", "com.instagram.android:id/action_icon") #TIEMPO DE ESPERA DE TIMEOUT DE ENVIO DE IMAGEN
    except Exception as e:
    ...
    try:
        enviar_contenido_V(driver, METRICAS["VIDEO"], 5, "com.instagram.android:id/direct_media_send_button", "com.instagram.android:id/action_icon") #TIEMPO DE ESPERA DE TIMEOUT DE ENVIO DE VIDEO ESTE ES DE 25 SEG
    except Exception as e:
