...
class PrinterPanel(FloatLayout):
    ...
    def iniciar(self, instance):
        ...
        if not os.path.exists(pid_file):
            ...
            if not pid.isdigit() or not os.path.exists(f"/proc/{pid}"):
                print("[INFO] Reanudando: solo union.py")
                subprocess.Popen(["bash", "/home/pi/Desktop/Finale/run_union.sh"])
            else:
                print("[INFO] union.py ya está corriendo.")
    ...
    def pausar(self, instance):
        ...
        # 1) Matar hijos
        children_pid_file = "/home/pi/union_children.pid"
        if os.path.exists(children_pid_file):
            with open(children_pid_file, "r") as f:
                for line in f:
                    pid = line.strip()
        ...
        # 2) Matar padre union.py si existe
        parent_pid_file = "/home/pi/union.pid"
        if os.path.exists(parent_pid_file):
            with open(parent_pid_file, "r") as f:
                ppid = f.read().strip()
            if ppid.isdigit() and os.path.exists(f"/proc/{ppid}"):
                os.kill(int(ppid), signal.SIGTERM)
                print(f"[INFO] Proceso padre union.py detenido (PID {ppid}).")

        # 3) Limpiar archivos PID
        for p in (children_pid_file, parent_pid_file):
            try:
                if os.path.exists(p):
                    os.remove(p)
            except Exception:
        ...
        # 5) Detener los 3 servicios en paralelo a la UI
        threading.Thread(target=_stop_gps_services, daemon=True).start()

    def reiniciar(self, instance):
        # Indicadores a blanco
        for ind in self.indicators:
            ind.set_color((1, 1, 1, 1))  # Blanco

        # Cambio a verde en 10s
        Clock.schedule_once(lambda dt: [ind.set_color((0, 1, 0, 1)) for ind in self.indicators], 10)
        ...
        # Terminar proceso actual si sigue vivo
        if self.process and self.process.poll() is None:
            self.process.terminate()
            self.process.wait()

        # Relanzar start_test.sh
        log_path = os.path.expanduser("~/Desktop/drivetest.log")
        with open(log_path, "w") as log_file:
            self.process = subprocess.Popen(
                ["bash", "/home/pi/Desktop/Finale/start_test.sh"],
    ...
    def salir(self, instance):
        ...
        # Reboot en 5s
        subprocess.Popen(["bash", "-lc", "sleep 5; sudo reboot"])

        # Cerrar la app
        App.get_running_app().stop()

# --- Fuera de la clase: detener GpsService en 3 dispositivos ---
def _stop_gps_services():
    try:
        subprocess.run(
            ["adb", "-s", "RF8MB0G4KTJ", "shell", "am", "stopservice", "-n", "viva.vast/.GpsService"],
            capture_output=True, text=True, timeout=4
        )
        print("[INFO] GpsService detenido en RF8MB0G4KTJ")

        subprocess.run(
            ["adb", "-s", "R58MA32XQQW", "shell", "am", "stopservice", "-n", "viva.vast/.GpsService"],
            capture_output=True, text=True, timeout=4
        )
        print("[INFO] GpsService detenido en R58MA32XQQW")

        subprocess.run(
            ["adb", "-s", "R58M795NHZF", "shell", "am", "stopservice", "-n", "viva.vast/.GpsService"],
            capture_output=True, text=True, timeout=4
        )
        print("[INFO] GpsService detenido en R58M795NHZF")
...
class PrinterApp(App):
    def build(self):
        # Forzar always on top
        Clock.schedule_once(lambda dt: self.set_always_on_top(), 0.5)
        return PrinterPanel()

    def set_always_on_top(self):
        try:
            wid = Window._get_system_handle()
            subprocess.Popen([
                "xprop", "-id", str(wid), "-f", "_NET_WM_STATE", "32a",
                "-set", "_NET_WM_STATE", "_NET_WM_STATE_ABOVE"
            ])
        except Exception as e:
            print("[WARN] No se pudo forzar always on top:", e)
